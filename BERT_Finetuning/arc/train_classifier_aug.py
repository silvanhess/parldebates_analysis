...
# ============================================================================
# SECTION 14: SWEEP CONFIGURATION (UPDATE THIS IF YOU HAVE IT)
# ============================================================================

sweep_config = {
    'name': 'roberta_augmented_sweep',
    'method': 'bayes',
    'metric': {
        'name': 'f1_macro',
        'goal': 'maximize'
    ...
    'parameters': {
        'num_train_epochs': {
            'values': [5, 7, 10]  # ✅ INCREASED (was probably 2, 3, 5)
        },
        'train_batch_size': {
            'values': [4, 8]  # Keep smaller for augmented data
        },
        'learning_rate': {
        ...
        'weight_decay': {
            'values': [0.05, 0.1, 0.15, 0.2]  # ✅ ADDED for regularization
        },
        'hidden_dropout_prob': {
            'values': [0.2, 0.3, 0.4]  # ✅ ADDED
        },
        'stride': {
            'values': [0.7, 0.8, 0.9]  # ✅ ADDED to optimize augmentation
        },
        'max_seq_length': {
            'values': [128, 256, 512]  # ✅ ADDED to test different window sizes
        }
    }
}

print("✅ Sweep configuration updated for augmented data")
...
# ============================================================================
# SECTION 13: MODEL CONFIGURATION WITH SLIDING WINDOW AUGMENTATION
# ============================================================================
...
# ============================================================================
# ✅ TRAINING PARAMETERS (ADJUSTED FOR AUGMENTED DATA)
# ============================================================================
model_args.num_train_epochs = 5  # ✅ INCREASED from 2 (more data needs more epochs)
...
# ============================================================================
model_args.sliding_window = True  # ✅ ENABLED
model_args.stride = 0.8  # ✅ ADDED: 80% overlap (more augmentation)
# stride=0.8 means each window starts at 80% of previous window's position
# Higher stride (0.8-0.9) = more overlap = more samples
# Lower stride (0.5) = less overlap = fewer samples

# ============================================================================
# ✅ REGULARIZATION (ADDED TO PREVENT OVERFITTING WITH MORE DATA)
# ============================================================================
model_args.weight_decay = 0.1  # ✅ ADDED: L2 regularization
model_args.warmup_ratio = 0.1  # ✅ ADDED: Gradual learning rate warmup
model_args.hidden_dropout_prob = 0.3  # ✅ ADDED: Dropout in hidden layers
model_args.attention_probs_dropout_prob = 0.3  # ✅ ADDED: Attention dropout

# ============================================================================
# ✅ EVALUATION SETTINGS (ADJUSTED FOR MORE TRAINING STEPS)
# ============================================================================
model_args.evaluate_during_training_steps = 150  # ✅ INCREASED from default
# With augmentation, you'll have ~3x more batches per epoch
# Evaluate every 150 steps instead of every 50

# ============================================================================
# ✅ EARLY STOPPING (ADJUSTED FOR AUGMENTED DATA)
# ============================================================================
model_args.use_early_stopping = True
model_args.early_stopping_patience = 3  # ✅ INCREASED from 2 (give more time)
...
model_args.wandb_project = "roberta_multiclass_500samples_augmented"  # ✅ UPDATED
model_args.fp16 = False
model_args.weight = weights.tolist()  # Class weights
...
print(f"   Stride: {model_args.stride} (80% overlap)")
print("\n📊 REGULARIZATION")
print(f"   Weight decay: {model_args.weight_decay}")
print(f"   Dropout: {model_args.hidden_dropout_prob}")
print(f"   Warmup ratio: {model_args.warmup_ratio}")
print("\n🛑 EARLY STOPPING")
print(f"   Patience: {model_args.early_stopping_patience}")
print(f"   Delta: {model_args.early_stopping_delta}")
print(f"   Metric: {model_args.early_stopping_metric}")
print("="*60)
...
avg_text_length = train_df['text'].str.split().str.len().mean()

# Calculate expected number of windows
tokens_per_window = model_args.max_seq_length
stride_tokens = int(tokens_per_window * model_args.stride)

# Estimate windows per sample
estimated_windows = max(1, int(avg_text_length / stride_tokens))

print(f"Average text length: {avg_text_length:.0f} words")
print(f"Window size: {tokens_per_window} tokens")
print(f"Stride: {stride_tokens} tokens")
print(f"Estimated windows per sample: ~{estimated_windows}")
print("\n📊 Dataset Size Impact:")
print(f"   Original training samples: {len(train_df)}")
print(f"   Estimated augmented samples: ~{len(train_df) * estimated_windows}")
print(f"   Increase: ~{estimated_windows}x")
print("\n⏱️ Training Time Impact:")
print(f"   Original steps per epoch: ~{len(train_df) // model_args.train_batch_size}")
print(f"   Estimated steps per epoch: ~{(len(train_df) * estimated_windows) // model_args.train_batch_size}")
print(f"   Total training steps: ~{((len(train_df) * estimated_windows) // model_args.train_batch_size) * model_args.num_train_epochs}")
print("="*60)
...
def train():
    """
    Training function for hyperparameter sweep with sliding window augmentation
    - Creates and trains model with current sweep configuration
    - Evaluates on validation set (NOT test set)
    - Logs comprehensive metrics to wandb
    - Tracks augmentation effects
    """
    ...
    # ✅ ADDED: Display augmentation status
    print("\n" + "="*60)
    print("📈 DATA AUGMENTATION STATUS")
    print("="*60)
    print(f"Sliding window: {model_args.sliding_window}")
    if model_args.sliding_window:
        print(f"Stride: {model_args.stride}")
        print(f"Max seq length: {model_args.max_seq_length}")
        
        # Estimate augmentation factor
        avg_text_length = train_df['text'].str.split().str.len().mean()
        stride_tokens = int(model_args.max_seq_length * model_args.stride)
        estimated_windows = max(1, int(avg_text_length / stride_tokens))
        
        print(f"Original training samples: {len(train_df)}")
        print(f"Estimated augmented samples: ~{len(train_df) * estimated_windows}")
        print(f"Augmentation factor: ~{estimated_windows}x")
    else:
        print("❌ Sliding window disabled - no augmentation")
    print("="*60)
    ...
    model.train_model(
        train_df,
        eval_df=val_df,  # ✅ FIXED: Use val_df, NOT test_df
        accuracy=accuracy,
        f1_macro=f1_macro,
    ...
    true_labels = val_df['final_climate'].values  # ✅ FIXED: Correct column name
    ...
    for i in range(3):
        ...
        class_metrics[f'class_{i}_accuracy'] = class_acc
        class_metrics[f'class_{i}_precision'] = report[f'Class {i}']['precision']
        class_metrics[f'class_{i}_recall'] = report[f'Class {i}']['recall']
        class_metrics[f'class_{i}_f1'] = report[f'Class {i}']['f1-score']
        class_metrics[f'class_{i}_support'] = int(report[f'Class {i}']['support'])
        
        print(f"Class {i}: Acc={class_acc:.4f}, "
              f"Prec={class_metrics[f'class_{i}_precision']:.4f}, "
              f"Rec={class_metrics[f'class_{i}_recall']:.4f}, "
              f"F1={class_metrics[f'class_{i}_f1']:.4f}, "
              f"Support={class_metrics[f'class_{i}_support']}")
    print("="*60)
    
    # ✅ ADDED: Calculate training statistics
    actual_training_steps = len(model.results) if hasattr(model, 'results') else 0
    expected_steps_no_aug = (len(train_df) // model_args.train_batch_size) * model_args.num_train_epochs
    
    if actual_training_steps > 0:
        actual_augmentation_factor = actual_training_steps / expected_steps_no_aug
        print("\n📊 Training Statistics:")
        print(f"   Expected steps (no augmentation): {expected_steps_no_aug}")
        print(f"   Actual training steps: {actual_training_steps}")
        print(f"   Actual augmentation factor: {actual_augmentation_factor:.2f}x")
    else:
        actual_augmentation_factor = 1.0
    
    # ✅ ADDED: Calculate class imbalance metrics
    class_distribution = pd.Series(true_labels).value_counts(normalize=True).sort_index()
    class_imbalance_ratio = class_distribution.max() / class_distribution.min()
    
    print("\n📊 Class Distribution in Validation Set:")
    for cls, pct in class_distribution.items():
        print(f"   Class {cls}: {pct:.2%}")
    print(f"   Imbalance ratio: {class_imbalance_ratio:.2f}")
    
    # ✅ IMPROVED: Comprehensive wandb logging
    wandb.log({
        # Main metrics
        ...
        **class_metrics,  # ✅ ADDED: Unpack all class metrics
        
        # ✅ ADDED: Augmentation metrics
        'sliding_window_enabled': model_args.sliding_window,
        'stride': model_args.stride if model_args.sliding_window else 0,
        'actual_augmentation_factor': actual_augmentation_factor,
        'actual_training_steps': actual_training_steps,
        'expected_steps_no_aug': expected_steps_no_aug,
        ...
        'time_per_sample_seconds': training_time / len(train_df),  # ✅ ADDED
        
        # ✅ ADDED: Model configuration
        'device': 'cpu',
        'num_epochs': model_args.num_train_epochs,
        'batch_size': model_args.train_batch_size,
        'learning_rate': model_args.learning_rate,
        'max_seq_length': model_args.max_seq_length,
        'weight_decay': model_args.weight_decay if hasattr(model_args, 'weight_decay') else 0,
        'dropout': model_args.hidden_dropout_prob if hasattr(model_args, 'hidden_dropout_prob') else 0,
    })
    
    # ✅ ADDED: Log confusion matrix as image
    ...
    ax.set_ylabel('True')
    ax.set_title(f'Confusion Matrix - {wandb.run.name}')
    
    # Log to wandb
    wandb.log({"confusion_matrix": wandb.Image(fig)})
    plt.close(fig)
    
    # ✅ ADDED: Log per-class F1 comparison
    fig, ax = plt.subplots(figsize=(10, 6))
    classes = ['Class 0', 'Class 1', 'Class 2']
    f1_scores = [class_metrics[f'class_{i}_f1'] for i in range(3)]
    support = [class_metrics[f'class_{i}_support'] for i in range(3)]
    
    x = np.arange(len(classes))
    width = 0.35
    
    ax.bar(x - width/2, f1_scores, width, label='F1 Score', color='steelblue')
    ax.bar(x + width/2, [s/max(support) for s in support], width, 
           label='Normalized Support', color='lightcoral')
    
    ax.set_xlabel('Class')
    ax.set_ylabel('Score')
    ax.set_title('Per-Class F1 Scores and Support Distribution')
    ax.set_xticks(x)
    ax.set_xticklabels(classes)
    ax.legend()
    ax.grid(axis='y', alpha=0.3)
    
    wandb.log({"per_class_metrics": wandb.Image(fig)})
    plt.close(fig)
    ...
    print(f"\n{'='*60}")
    print(f"✅ Training run completed: {wandb.run.name}")
    print(f"{'='*60}\n")
