...
term_doc_freq <- col_sums(dtm_german > 0)
...
# remove rare terms
# dtm_german_cleaned <- removeSparseTerms(dtm_german, 0.99)
term_doc_freq <- col_sums(dtm_french > 0)
...
start <- Sys.time()
set.seed(123)
lda_model_german <- LDA(dtm_german_cleaned, k = 5)
end <- Sys.time()
duration_german <- end - start
lda_topics_german <- tidy(lda_model, matrix = "beta")

start <- Sys.time()
set.seed(123)
lda_model_french <- LDA(dtm_french_cleaned, k = 5)
end <- Sys.time()
duration_french <- end - start
lda_topics_french <- tidy(lda_model, matrix = "beta")
